...
const Dashboard = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setShortlinks((prevLinks) => [
          ...prevLinks,
          { originalUrl: data.originalUrl, shortUrl: data.shortUrl, _id: data._id },
        ]);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/edit/${shortUrl}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleDelete = async (shortUrl: string) => {
    ...
    if (confirm.isConfirmed) {
      ...
      try {
        const response = await fetch(`http://localhost:5000/api/delete/${shortUrl}`, {
          method: "DELETE",
        });
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <form onSubmit={handleCreateShortlink} className="mb-4">
        <label className="block text-lg mb-2">Enter the URL you want to shorten:</label>
        <input
          type="url"
          value={newUrl}
        ...
        <label className="block text-lg mb-2">Enter a custom short URL (optional):</label>
        <input
          type="text"
          value={newShortUrl}
      ...
      {shortlinks.length > 0 && (
        <div className="mt-6">
          ...
          <ul>
            {shortlinks.map((link) => (
              <li key={link._id} className="flex items-center justify-between mb-4">
                <div>
